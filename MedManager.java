...
public class MedManager {
	 ...
	 */
	HashSet <String> specNoRep = new HashSet<>();
	public void addSpecialities(String... specialities) {
		Integer n = specialities.length;
		List<String> listSpec = new ArrayList<>();
		for(int i = 0 ; i < n ; i++) {
			listSpec.add(specialities[i]);
		}
		for(int j = 0 ; j < n ; j++) {
			specNoRep.addAll(listSpec);
		}
	...
	HashMap <String, Doctor> docId2Doc = new HashMap<>();
	public void addDoctor(String id, String name, String surname, String speciality) throws MedException {
		if(docId2Doc.containsKey(id))
			throw new  MedException("doc already exists");
		else if(speciality == null) {
			throw new  MedException("no spec given");
		}
		else {
	...
	public Collection<String> getSpecialists(String speciality) {
		...
		List<String> docs = docId2Doc.entrySet()
                .stream().filter(entry -> entry.getValue().getSpec().equals(speciality))
	...
	public String getDocName(String code) {
		String s = docId2Doc.get(code).code2name(code);
		if (s != null)
			return s;
		else{return null;}
	}
	...
	public String getDocSurname(String code) {
		String s = docId2Doc.get(code).code2surname(code);
		if (s != null)
			return s;
		else{return null;}
	}
	...
	Map<String, List<String>> docWithSlots = new HashMap<>();
	public int addDailySchedule(String code, String date, String start, String end, int duration) {
		
		 String s = String.format("[%s-%s]", start, end);
		 Doctor d = docId2Doc.get(code);
		 d.addSlots(date, s);
		
		 if(docWithSlots.containsKey(code)) {
			 d.addSlots(date, s);
			 docWithSlots.put(code, d.giveSlotList());
			 return d.slots();
		 }
		 else {
			 List<String> l = new ArrayList<>();
			 l.add(s);
			 docWithSlots.put(code, l);
			 return d.slots();
		 }
	...
	public Map<String, List<String>> findSlots(String date, String speciality) {
		 Collection<String> d = getSpecialists(speciality); // doctors that have slots with this spec
		 List<String> selected = docWithSlots.entrySet()
         .stream().filter(entry -> d.contains(entry.getKey())) //
         .map(Map.Entry::getKey)
         .collect(Collectors.toList());
		 //filter selected doctors with matching date
		 //docId2Doc.keySet().stream().filter(d -> selected.contains(d);)
		 //.getValue().filter(v -> v.giveDate2Slot().containsKey(date));
		 return null;
	...
	public String setAppointment(String ssn, String name, String surname, String code, String date, String slot) throws MedException {
		if( !docId2Doc.containsKey(code)) {throw new MedException("NOT A VALID DOCTOR");}
		//if findSlots.get(
		return null;
	}
